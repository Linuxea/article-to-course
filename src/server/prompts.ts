...
1. paragraph — 正文段落，支持内联术语解释。**这是课程的主要载体，一节里 paragraph 的数量应占整节 block 总数的 70% 以上。**
   { "type":"paragraph", "segments":[ {"type":"text","text":"..."} , {"type":"term","text":"术语","definition":"1-2 句通俗解释"} ] }
   规则：专业术语在每节第一次出现时必须用 term 包裹；每段 4-8 句，逐步展开一个观点、给出例子或推理过程（而非只甩结论）。
   相邻 paragraph 之间要有论证链：用"首先""进而""因此""由此可见""换言之""举个例子"等论述式过渡词衔接，让多段读起来像连贯的论述，而不是孤立的卡片。
...
7. keypoints — 要点卡片网格（总结/清单）。
   ...
   使用条件：仅在本节末尾做要点回顾、且要点数 ≤3 时使用；不得用它替代正文论证——该讲透的概念请用 paragraph。

8. steps — 编号步骤卡片。
   { "type":"steps", "items":[{"title":"标题","body":"描述"}] }
   使用条件：仅在读者需要"按顺序执行的操作步骤"时使用；概念性讲解、原理阐述请用 paragraph，不得拆成步骤卡片。
...
- 产出 3 到 5 个 screen；每个 screen 以 paragraph 为主体（通常 2-4 个连贯的 paragraph），仅在需要时插入 1 个交互或结构化 block。**整节 paragraph 总数应明显多于其他 block 之和（目标 70%-90%）。**
- 这一节里至少出现 1 个【交互】元素（quiz / chat / flow / arch 之一），根据内容需要可以安排 2-3 个；其余以 paragraph 为主，callout / translation / keypoints / steps / table 仅在 BLOCK_CATALOG 中注明的使用条件下才用。
- 涉及多方对比、优缺点、参数差异时，优先用 table；涉及系统组成、模块职责时，优先用 arch；除此之外尽量用 paragraph 讲透。
- 写作风格：以严谨、书面的论述风格系统讲解——先铺垫背景，再逐步展开论证，辅以恰当的例子或推理过程，最后点明意义。相邻 paragraph 之间用"首先""进而""因此""由此可见"等论述式过渡词衔接，不要把段落写成彼此孤立的卡片。
- 表达准确、通俗易懂但不口语化，每段 4-8 句，把概念讲透、给出例子或推理过程（而不是只给结论）；专业术语第一次出现时必须用 paragraph 的 term 包裹并给 1-2 句通俗定义。
